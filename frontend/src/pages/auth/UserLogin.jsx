...
const UserLogin = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!validateEmail(email)) {
      toast.error("Please enter your Gmail address (e.g., example@gmail.com)");
      return;
    }
    if (!password) {
    ...
    if (!validatePassword(password)) {
      toast.error(
        "Invalid password format. Your password should have:\n• Minimum 8 characters\n• At least one uppercase letter\n• At least one lowercase letter\n• At least one number\n• At least one special character"
      );
      return;
    }
        ...
        <form className="space-y-4" onSubmit={handleSubmit} noValidate>
          ...
          <div className="flex items-center justify-between text-sm">
            <label className="flex items-center space-x-2 text-gray-600">
              <input
                type="checkbox"
                className="h-4 w-4 text-green-600 border-gray-300 rounded focus:ring-green-500 hover:cursor-pointer"
              />
